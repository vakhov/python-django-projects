...
def change_seo_slug(parent, new_parrent):
    """
    -parent - Строка содержащий путь родителя
    -new_parent - Строка содержащая путь нового родителя
    """
    from seo.models import Metatag
    seo_path = Metatag.objects.filter(name__startswith=parent)
    for path in seo_path:
        path.name = path.name.replace(parent, new_parrent)
        path.save()
...
class Section(SortableModel):
    ...
    def change_slug(self, slug):
        ...
        change_seo_slug(old_path, self.path)
    ...
    def change_parent(self, parent, order=None):
        ...
        for child in children:
            child.path = child.path.replace(old_path, self.path)
            child.save()
        change_seo_slug(current_parent.path, parent.path)
